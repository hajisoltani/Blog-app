...
function PaginationNumber({ page, href, isActive, position }) {
  const className = classNames(
    "flex h-10 w-10 items-center justify-center text-sm border border-secondary-400 text-secondary-600",
    {
...
function PaginationArrow({ href, direction, isDisabled }) {
  ...
  return isDisabled ? (
    ...
    <Link className={`text-secondary-600 ${className}`} href={href}>
      {icon}
    </Link>
  );
}
